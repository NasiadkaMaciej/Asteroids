...
struct GameSettings {
	...
	// Use map instead
	unsigned int frames = 60, resX = sf::VideoMode::getFullscreenModes().front().size.x,
				 resY = sf::VideoMode::getFullscreenModes().front().size.y, antialias = 0;
	bool vsync = true, fs = true, sfx = true, music = true, background = true;
	void loadSettings();
	void saveSettings();
	sf::State translateFS();
	bool checkRes();
